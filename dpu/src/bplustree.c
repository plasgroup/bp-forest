...
static unsigned INIT_nodes(void)
{
    ...
    for (;; tmp_height++, nr_nodes_in_lower += nr_nodes) {
        ...
        if (nr_nodes == 1) {
            ...
            return nr_nodes_in_lower + nr_nodes;
        }
...
static void INIT_barrier_init_tasklets(void)
{
    if (me() != 0) {
        wait_for_prev_ready();
    }
    if (me() != TASK_INIT_NR_TASKLETS - 1) {
        notify_next_of_readiness();
        wait_for_next_ready();
    }
    if (me() != 0) {
...
void task_init(void)
{
    ...
    if (me() < TASK_INIT_NR_TASKLETS) {
        const unsigned nr_nodes = INIT_nodes();

        INIT_barrier_init_tasklets();

        _Static_assert(TASK_INIT_ALLOC_NR_TASKLETS <= TASK_INIT_NR_TASKLETS, "TASK_INIT_ALLOC_NR_TASKLETS <= TASK_INIT_NR_TASKLETS");
        Allocator_init(nr_nodes);
