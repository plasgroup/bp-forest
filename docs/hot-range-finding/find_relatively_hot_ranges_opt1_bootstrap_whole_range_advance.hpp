/// @file find_relatively_hot_ranges_opt1_bootstrap_whole_range_advance.hpp
/// @brief 論文 Algorithm 3 第2スキャンの sliding window 最適化版 + opt1。
///        ベースは find_relatively_hot_ranges_sliding_window.hpp。
///        ビルド対象ではない (参照用)。
///
...
///
/// 含まない最適化 (後続 opt で追加):
///   - opt2: BootstrapAllHotEarlyExit (bootstrap で全 range 吸収した場合の早期 return)
...
namespace relatively_hot_ranges_opt1_bootstrap_whole_range_advance_detail {
...
// ── Algorithm 3 第2スキャン: argmax + carve-out (opt1 版) ──
...
find_relatively_hot_ranges(const LinkedList<ChunkedPairsRange>::iterator begin_range, const LinkedList<ChunkedPairsRange>::iterator end_range,
    ...
    Func&& hot_hook)
{
    using namespace relatively_hot_ranges_opt1_bootstrap_whole_range_advance_detail;
    ...
    if (bootstrapped_any_range) {
        ...
        if (bootstrap_r_range == end_range) {
            // 全 range を whole-range bootstrap で吸収した。
            // opt1 では early return しない (opt2 の責務)。
            // 最後の counted 位置 prev(end_range)->end() を sentinel にし、
            // 既存 outer loop をそのまま終端まで流す (inner loop は r == end() で skip)。
            bootstrap_r_range = std::prev(end_range);
            bootstrap_r = bootstrap_r_range->end();
            argmax_right_range = bootstrap_r_range;
            argmax_right = bootstrap_r;

            if (bootstrap_r_range != l_range) {
                // 最後の吸収 range を middle から right partial へ降格する。
                // 不変条件: right_window_offcut は既に non_left_effective に含まれている
                //          middle 寄与 (slot_rounded(full range)) と等しい値なので、
                //          non_left_effective の総和は変わらず、ラベルだけ更新される。
