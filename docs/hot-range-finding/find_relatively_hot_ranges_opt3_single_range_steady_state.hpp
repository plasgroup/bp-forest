/// @file find_relatively_hot_ranges_opt3_single_range_steady_state.hpp
/// @brief 論文 Algorithm 3 第2スキャンの sliding window 最適化版 + opt1 + opt2 + opt3。
///        ベースは find_relatively_hot_ranges_opt2_bootstrap_all_hot_early_exit.hpp。
///        ビルド対象ではない (参照用)。
///
...
namespace relatively_hot_ranges_opt3_single_range_steady_state_detail {
...
// ── Algorithm 3 第2スキャン: argmax + carve-out (opt3 版) ──
...
find_relatively_hot_ranges(const LinkedList<ChunkedPairsRange>::iterator begin_range, const LinkedList<ChunkedPairsRange>::iterator end_range,
    ...
    Func&& hot_hook)
{
    using namespace relatively_hot_ranges_opt3_single_range_steady_state_detail;
         ...
         r_range != end_range; ++r_range)
    {
             ...
             r != r_range->end(); ++r)
        {
            ...
            if (l_range == r_range) {
                ...
                // Step A (multi-range): right_npairs_offcut + non_left_effective を差分更新。
                nqrys_in_window += r->load();
