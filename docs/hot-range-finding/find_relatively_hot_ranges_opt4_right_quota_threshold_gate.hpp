/// @file find_relatively_hot_ranges_opt4_right_quota_threshold_gate.hpp
/// @brief 論文 Algorithm 3 第2スキャンの sliding window 最適化版 + opt1..opt4。
///        ベースは find_relatively_hot_ranges_opt3_single_range_steady_state.hpp。
///        ビルド対象ではない (参照用)。
///
/// ── opt4: RightQuotaThresholdGate ───────────────────
/// bpforest.ipp `find_relatively_hot_ranges` の right-quota 閾値ゲート (`right_npairs_offcut > right_window_offcut`) 相当。
///
/// 目的:
...
/// bpforest.ipp `find_relatively_hot_ranges` メインループの single-range 分岐 (`left_range == right_range` fast path) 相当。
///
/// 目的:
...
namespace relatively_hot_ranges_opt4_right_quota_threshold_gate_detail {
...
// ── Algorithm 3 第2スキャン: argmax + carve-out (opt4 版) ──
...
find_relatively_hot_ranges(const LinkedList<ChunkedPairsRange>::iterator begin_range, const LinkedList<ChunkedPairsRange>::iterator end_range,
    ...
    Func&& hot_hook)
{
    using namespace relatively_hot_ranges_opt4_right_quota_threshold_gate_detail;
