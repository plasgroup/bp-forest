...
#define NUM_INIT_TREES_IN_DPU (NUM_SEAT_IN_A_DPU / 4)
...
key_int64_t tree_to_key_map[NR_DPUS][NUM_SEAT_IN_A_DPU];
int key_index[NR_DPUS][NUM_SEAT_IN_A_DPU + 1] = {0};
BPTreeNode nodes_buffer[MAX_NODE_NUM];
uint64_t nodes_num;
std::pair<int,int> migration_dest[NR_DPUS][NUM_SEAT_IN_A_DPU];
split_info_t split_result[NR_DPUS][NUM_SEAT_IN_A_DPU];
dpu_result_t dpu_results[NR_DPUS][MAX_REQ_NUM_IN_A_DPU];
...
{
    for (int i = 0; i < NR_DPUS; i++) {
        for (int j = 0; j < NUM_SEAT_IN_A_DPU; j++)
...
void receive_results(struct dpu_set_t set, struct dpu_set_t dpu){
    ...
    {
        DPU_ASSERT(dpu_prepare_xfer(
            dpu, &dpu_results[each_dpu]));
    }
    DPU_ASSERT(dpu_push_xfer(set, DPU_XFER_FROM_DPU, "result", 0,
        sizeof(dpu_result_t) * send_size,
        DPU_XFER_DEFAULT));
}

#ifdef DEBUG_ON
// void check_results(dpu_result_t dpu_results[NR_DPUS][MAX_REQ_NUM_IN_A_DPU], int key_index[NR_DPUS][NUM_SEAT_IN_A_DPU + 1]){
//     for(int dpu = 0; dpu < NR_DPUS; dpu++){
//         for(int index = key_index[dpu][])
//     }
// }
#endif
...
void initialize_dpus(int num_init_reqs, struct dpu_set_t set, struct dpu_set_t dpu)
{
    ...
    //printf("interval = %ld\n", interval);
    int num_keys_for_tree[NR_DPUS][NUM_SEAT_IN_A_DPU] = {0};
    int num_keys_for_DPU[NR_DPUS] = {0};
    //printf("NUM_SEAT_IN_A_DPU = %d\n", NUM_SEAT_IN_A_DPU);
    ...
    for (int i = 0; i < NR_DPUS; i++) {
        for (int j = 1; j <= NUM_SEAT_IN_A_DPU; j++) {
...
void recieve_split_info(struct dpu_set_t set, struct dpu_set_t dpu){
    ...
    DPU_ASSERT(dpu_push_xfer(set, DPU_XFER_FROM_DPU, "split_result", 0,
        sizeof(split_info_t) * NUM_SEAT_IN_A_DPU,
        DPU_XFER_DEFAULT));
}

void update_cpu_struct(){
    for(int dpu = 0; dpu < NR_DPUS; dpu++){
        for(int old_tree = 0; old_tree < NUM_SEAT_IN_A_DPU; old_tree++){
...
int batch_preprocess(std::ifstream& fs, int n, uint64_t &total_num_keys, int num_migration, struct dpu_set_t set, struct dpu_set_t dpu)
{
    ...
    PRINT_POSITION_AND_VARIABLE(key_count, %d)
    gettimeofday(&start, NULL);
    /* 1. それぞれの木へのクエリ数を数える */
    int num_keys_for_tree[NR_DPUS][NUM_SEAT_IN_A_DPU] = {0};
    int num_keys_for_DPU[NR_DPUS] = {0};
    ...
    while (num_trees_to_be_migrated) {
        ...
        if (__builtin_popcount(tree_bitmap[from_DPU]) > 1) {  // 木が1つだけだったら移動しない
            ...
            for (int i = 0; i < NUM_SEAT_IN_A_DPU; i++) {  // 最も負荷を分散出来る木を移動
...
int main(int argc, char* argv[])
{
    ...
    // std::cout << "NUM Trees per DPU(init):" << NUM_SEAT_IN_A_DPU << ", NUM Trees per DPU(max):" << NUM_SEAT_IN_A_DPU << ", num total trees:" << NUM_TOTAL_TREES << ", NR_DPUS:" << NR_DPUS << ", NR_TASKLETS:" << NR_TASKLETS << std::endl;
