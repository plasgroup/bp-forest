...
void execute_one_batch(dpu_set_t set1, dpu_set_t dpu1)
{
    ...
    for (int i = 0; i < MAX_NUM_BPTREE_IN_DPU; i++){
        if(split_ctx.split_info[i].new_tree_index >= 0) {
            int new_i = split_ctx.split_info[i].new_tree_index;
            key_to_tree_index.emplace(split_ctx.split_info[i].split_key, split_ctx.split_info[i].new_tree_index);
            printf("tree_%d(%d elems) was split into tree_%d(%d elems) and tree_%d(%d elems), border_key = %ld\n",i,split_ctx.num_elems[i]+split_ctx.num_elems[new_i], i,split_ctx.num_elems[i], new_i, split_ctx.num_elems[new_i],split_ctx.split_info[i].split_key);
        }
    }
}
